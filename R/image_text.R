...
#' @export
image_text <- function(x, x_labels = TRUE, y_labels = TRUE, in_labels = FALSE, ...){
  if(is.null(dim(x))) {
  ...
  if(x_labels) {
    ...
    mtext(text = colnames(x), side = 3, line = 0, at = at_v[seq(2, length(at_v), by = 2)])
  }
  if(y_labels) {
    if(is.null(rownames(x))) rownames(x) <- seq_len(nrow(x))
    mtext(text = rownames(x), side = 2, line = 0, at = at_h[rev(seq(2, length(at_h), by = 2))], las = 1)
  }
  if(in_labels) {
    xy <- as.matrix(expand.grid(x = at_v[seq(2, length(at_v), by = 2)], y = rev(at_h[seq(2, length(at_h), by = 2)])))
    text(xy[order(xy[, 1]), ], labels=x)

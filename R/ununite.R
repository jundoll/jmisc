...
#' @examples
#' iris %>% 
#'   group_by_unite(Petal.Width, Species) %>% 
#'   summarise(count = n()) %>% 
#'   ununite
#' iris %>% 
#'   group_by_unite(Petal.Width, Species, col_n = 'unite_name', sep = '|') %>% 
#'  summarise(count = n()) %>% 
#'  ununite
#' @export
ununite <- function(data, ...) {
  ...
  if(is.null(col_n) | is.null(origin_n) | is.null(sep)) {
    ...
    readr::type_convert(separate_(data = data, col = col_n, into = origin_n, sep = paste0('[', sep, ']'), ...))
  }
}